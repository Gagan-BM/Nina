export type MemoryChapter = {
  id: string;
  eyebrow: string;
  title: string;
  lines: string[];
  tone: "night" | "light" | "green" | "festival";
};

export const october2Chapters: MemoryChapter[] = [
  {
    id: "painting",
    eyebrow: "Chapter 01 · 02 October 2025",
    title: "The evening it all began. ❤️",
    lines: [
      "That afternoon, I was just painting. Nothing particularly special. I had absolutely no idea that the day was going to become one of the most important days in my story with you. ❤️",
      "When I later checked my photos from that day, I found the painting I had started. I had only just begun it on October 2nd; I finished it on October 3rd. 🎨🌙",
      "It was an evening scene: a single stag, and a bright moon. At that point, I was still single, minding my own business. 😂",
      "And then... you appeared that very evening. Like that bright moon suddenly showing up in the picture. 🌙❤️",
      "Maybe coincidence. Maybe just one of those beautiful little things life does. Either way, I like the story.",
    ],
    tone: "night",
  },
  {
    id: "call",
    eyebrow: "Chapter 02 · The unexpected call 📞",
    title: "One very casual “Okay.”",
    lines: [
      "Later that day, my cousin called and said they were going to Madikeri for Dasara. I had no idea what was actually going to happen.",
      "I knew there was a plan to visit, but I didn't know that your family would be hosting us and showing us around.",
      "I had seen Dasara in Madikeri years before, so I was free. He asked. I simply said: Okay. 😂",
      "That very normal Okay somehow became the beginning of something very not-normal.",
    ],
    tone: "light",
  },
  {
    id: "heard-about",
    eyebrow: "Chapter 03 · The first journey 🚗",
    title: "First stop: your home. ❤️",
    lines: [
      "Initially, I thought we were simply going by ourselves. Then we met the cousins in Kushalnagar, and I learned the plan: first your home. Then Dasara. 🌙✨",
      "I knew there was someone named Nidhi in the house. I had even seen your picture once before when my cousin showed it to me.",
      "Nothing dramatic. Just: Oh, okay.",
      "Little did I know that I was going to meet that person properly that very evening. ❤️",
    ],
    tone: "green",
  },
  {
    id: "first-sight",
    eyebrow: "Chapter 04 · Madikeri",
    title: "The first time I actually saw you.",
    lines: [
      "Then we reached Madikeri. And you got a call. You came to show us the way to your home.",
      "You got into the car through the opposite door from where I was sitting. You sat in the back seat, with another cousin between us.",
      "Honestly, I was curious. This was the person I had heard about, so I was quietly taking it all in.",
      "No idea what was coming next. And then we reached your home. ❤️",
    ],
    tone: "night",
  },
  {
    id: "coffee",
    eyebrow: "Chapter 05 · A completely innocent mistake ☕",
    title: "The Coffee Incident",
    lines: [
      "Then came my first coffee from you. And somehow, I received sugarless black coffee. 😂",
      "For someone with a serious sweet tooth, this was a rather unexpected first impression. Internally: What exactly is happening here? Externally: maintained dignity.",
      "I quietly drank it like it was the best coffee in the world. Meanwhile, everyone else had tasty coffee, and later the cousins were praising it. That made the situation even worse. 😂",
      "The funniest part? It was meant for your friend. So technically, I didn't even get my coffee. I'm still waiting for the proper sweet coffee. 🥲☕",
    ],
    tone: "light",
  },
  {
    id: "dasara",
    eyebrow: "Chapter 06 · Dasara walk 🌙✨",
    title: "Then we went for a walk.",
    lines: [
      "Then you took us around and showed us the Dasara Mantapas.",
      "Somewhere during that evening, I started noticing you differently. Not because of one dramatic moment. It was little things.",
      "The way you were friendly. The way you made everyone comfortable. The way you treated people. Your smile. Your warmth. Your cuteness.",
      "And yes... how pretty you looked. ❤️",
    ],
    tone: "festival",
  },
  {
    id: "thought",
    eyebrow: "Chapter 07 · The thought ❤️",
    title: "I would definitely like to know you better.",
    lines: [
      "Somewhere in between all of that, I had a very simple thought: I would definitely like to know her better.",
      "I didn't know where that thought was going. I didn't know what would happen next.",
      "I just knew I wanted to know you better. ❤️",
    ],
    tone: "green",
  },
  {
    id: "then-to-now",
    eyebrow: "Chapter 08 · From that evening to now ✨",
    title: "And then... us. ❤️",
    lines: [
      "A painting 🎨 A random call 📞 A simple Okay. A car ride 🚗 A first meeting ❤️ A slightly unfortunate coffee ☕😂 And a Dasara walk 🌙✨",
      "Eventually, it became us. ❤️",
      "From that first evening to today, we've come a long way. We've learned about each other: our quirks, our habits, our little memories.",
      "The things that make us laugh. The things that sometimes annoy us. 😂 And somewhere along the way, I fell in love with you. ❤️",
    ],
    tone: "light",
  },
  {
    id: "realization",
    eyebrow: "Chapter 09 · The realization 🌙",
    title: "I was walking into the beginning of our story.",
    lines: [
      "Painting. A call from my cousin. Okay. Kushalnagar. Madikeri. Your home. Coffee. Dasara. You. ❤️",
      "Sometimes I think about how casually that day started for me. I didn't know what I was painting into my life that evening. ❤️",
    ],
    tone: "night",
  },
];
