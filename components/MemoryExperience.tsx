...
const toneClasses = {
  night: "bg-[#4A1635] text-warm-white",
  light: "bg-warm-white text-cocoa",
  green: "bg-[#F5D9E4] text-cocoa",
  festival: "bg-[#6D1D49] text-warm-white",
};
...
function MemoryIntro({ onStart, onClose }: { onStart: () => void; onClose: () => void }) {
  return <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[#4A1635] px-5 py-8 text-warm-white"><MemoryParticles tone="night" /><button type="button" onClick={onClose} className="absolute left-5 top-5 z-10 inline-flex min-h-11 items-center gap-2 rounded-full border border-gold/40 px-4 text-sm font-bold outline-none transition hover:bg-white/10 focus-visible:ring-4 focus-visible:ring-white/30"><ArrowLeft className="h-4 w-4" /> Home</button><motion.section className="relative z-10 max-w-xl text-center" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}><Moon className="mx-auto h-12 w-12 text-champagne" fill="currentColor" aria-hidden="true" /><p className="mt-8 text-sm font-black uppercase tracking-[0.24em] text-gold">02 October 2025</p><h1 className="mt-4 text-4xl font-black leading-tight sm:text-6xl">The evening it all began. ❤️</h1><p className="mt-8 text-xl font-black text-petal">I thought it was going to be just another evening. I was wrong.</p><button type="button" onClick={onStart} className="mt-12 inline-flex min-h-14 items-center gap-2 rounded-full bg-nina-green px-7 text-base font-black text-white shadow-[0_0_36px_rgba(142,36,87,0.55)] outline-none transition hover:bg-nina-red focus-visible:ring-4 focus-visible:ring-white/35">Take me back <ArrowRight className="h-5 w-5" /></button></motion.section></main>;
}

function PaintingVisual() { return <div className="mb-8 overflow-hidden rounded-xl border border-gold/50 bg-[#301525] p-5 shadow-2xl"><div className="relative h-44 overflow-hidden rounded-lg bg-[radial-gradient(circle_at_72%_25%,#E8D2B0_0_10%,transparent_11%),linear-gradient(180deg,#582146_0%,#4A1635_58%,#301525_100%)]"><span className="absolute left-[14%] top-[22%] text-gold/80">✦</span><span className="absolute left-[45%] top-[13%] text-warm-white/75">✦</span><span className="absolute left-[84%] top-[46%] text-gold/70">·</span><div className="absolute bottom-0 left-[28%] h-24 w-12 rounded-t-[100%] border-x-4 border-t-4 border-champagne" /><div className="absolute bottom-16 left-[27%] h-9 w-9 rounded-full bg-champagne" /><div className="absolute bottom-[5.2rem] left-[24%] h-8 w-1 rotate-[-25deg] bg-champagne" /><div className="absolute bottom-[5.2rem] left-[31%] h-8 w-1 rotate-[25deg] bg-champagne" /></div><p className="mt-3 text-xs font-bold text-champagne">Started on October 2. Finished on October 3. 🎨🌙</p></div>; }
...
function FestivalVisual() { return <div className="mb-8 flex h-20 items-end justify-around overflow-hidden rounded-xl bg-[#6D1D49] px-5"><Sparkles className="mb-10 h-5 w-5 text-gold" /><span className="mb-8 text-warm-white">✦</span><Sparkles className="mb-7 h-4 w-4 text-champagne" /><span className="mb-12 text-gold">✦</span></div>; }
...
function CoffeeInteraction({ answer, onAnswer }: { answer: "black" | "sugar" | null; onAnswer: (answer: "black" | "sugar") => void }) { return <section className="mt-8 rounded-xl border border-nina-red/15 bg-white/75 p-5 text-cocoa"><p className="text-sm font-black uppercase tracking-[0.16em] text-nina-red">What would Gagan normally choose?</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => onAnswer("black")} className="min-h-12 rounded-lg border border-deep-red/15 px-4 font-black outline-none transition hover:border-nina-red focus-visible:ring-4 focus-visible:ring-nina-red/25">Black coffee</button><button type="button" onClick={() => onAnswer("sugar")} className="min-h-12 rounded-lg bg-nina-green px-4 font-black text-white outline-none transition hover:bg-deep-red focus-visible:ring-4 focus-visible:ring-nina-green/25">Coffee + sugar</button></div>{answer ? <p className="mt-4 font-bold" aria-live="polite">{answer === "sugar" ? "Exactly. You know me. ❤️" : "Bold choice. Unfortunately, historically inaccurate. 😂"}</p> : null}</section>; }
function CoffeeDebt() { return <aside className="mt-5 rounded-xl border border-gold/35 bg-petal p-5 text-cocoa"><div className="flex items-start gap-3"><Coffee className="mt-1 h-5 w-5 text-nina-green" /><div><p className="font-black">Coffee Debt: 1 proper cup. ❤️☕</p><p className="text-sm font-bold">Status: still pending</p><p className="mt-2 text-sm">I am still waiting for my proper sweet coffee from you.</p></div></div></aside>; }
...
function FinalMemory({ moonClicks, onMoonClick, onRelive, onSave, saved }: { moonClicks: number; onMoonClick: () => void; onRelive: () => void; onSave: () => void; saved: boolean }) { return <motion.section className="my-5 flex flex-1 items-center justify-center overflow-hidden rounded-2xl bg-[#4A1635] px-6 py-12 text-center text-warm-white shadow-soft" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><div className="max-w-2xl"><button type="button" onClick={onMoonClick} aria-label="A small moon" className="mx-auto rounded-full p-2 text-champagne outline-none transition hover:text-white focus-visible:ring-4 focus-visible:ring-white/35"><Moon className="h-10 w-10" fill="currentColor" /></button>{moonClicks >= 3 ? <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 rounded-xl border border-gold/35 bg-white/10 p-4"><p className="font-black text-champagne">You found the moon. 🌙</p><p className="mt-2 text-sm font-bold">Maybe it was coincidence. Maybe it was just a painting. Either way, I&apos;m really glad you appeared that evening. ❤️</p><p className="mt-2 text-xs font-bold text-petal">Okay, that&apos;s enough investigating. 😌</p></motion.div> : null}<p className="mt-6 text-sm font-black uppercase tracking-[0.22em] text-gold">02.10 · The evening it all began. ❤️</p><h1 className="mt-4 text-4xl font-black leading-tight sm:text-6xl">I hope I get to experience all the Dasaras yet to come with you.</h1><div className="mx-auto mt-8 max-w-lg space-y-3 text-base font-bold leading-relaxed sm:text-lg"><p>I still think it&apos;s funny that I spent that afternoon painting a moonlit evening without knowing that my own bright moon was going to appear that night. 🌙❤️</p><p>From that first Dasara to all the Dasaras yet to come...</p><p>And yes... I&apos;m still waiting for that proper coffee. ☕😂</p><p className="text-2xl">❤️</p></div>{saved ? <p className="mt-8 inline-flex items-center gap-2 rounded-full bg-petal px-5 py-3 font-black text-cocoa"><BookmarkCheck className="h-5 w-5 text-nina-green" /> Memory saved.</p> : <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row"><button type="button" onClick={onRelive} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-white/25 px-5 font-black outline-none transition hover:bg-white/10 focus-visible:ring-4 focus-visible:ring-white/30"><RotateCcw className="h-4 w-4" /> Relive October 2nd</button><button type="button" onClick={onSave} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-nina-green px-5 font-black outline-none transition hover:bg-nina-red focus-visible:ring-4 focus-visible:ring-white/30"><BookmarkCheck className="h-4 w-4" /> Keep this memory</button></div>}</div></motion.section>; }
